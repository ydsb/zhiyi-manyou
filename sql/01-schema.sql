...
(
    ...
    -- FR-M5-08 阶段性成果双向确认：DONE 只是"某人宣称完成"，confirmed 才是"对方认可"。
    -- 两者分开存储，因此"仅自述完成"与"已被对方确认"可区分（详见 sql/09-migration-m5-confirm.sql）。
    `confirmed`      TINYINT      NOT NULL DEFAULT 0 COMMENT '对方是否已确认该阶段成果：0未确认 1已确认',
    `confirmed_by`   VARCHAR(32)           DEFAULT NULL COMMENT '确认人学号',
    `confirmed_at`   DATETIME              DEFAULT NULL COMMENT '确认时间',
    `confirm_remark` VARCHAR(255)          DEFAULT NULL COMMENT '确认或提出异议的说明',
    `done_by`        VARCHAR(32)           DEFAULT NULL COMMENT '标记完成的人学号（打卡者）',
